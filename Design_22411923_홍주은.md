...
| :--- | :--- |
| `Member` | Jo:YUl을 사용하는 기본 회원을 표현한다. Supabase 사용자 ID, 학번, 이름, 이메일, 권한 정보를 가지며 팀 가입, 이벤트 투표 등의 기능을 수행한다. |
| `Administrator` | `Member`를 상속하는 관리자 클래스이다. 팀 생성, 팀원 초대, 일정 확정, 제출물 승인 같은 관리 기능을 수행한다. |
| `AuthAccount` | Supabase Auth와 연결되는 인증 계정 정보를 표현한다. 이메일, 비밀번호 해시, 마지막 로그인 시간, 인증 여부를 가진다. |
| `Notification` | 팀 초대, 일정 확정, 과제 제출, 이벤트 투표 요청 등 사용자에게 전달되는 알림을 표현한다. |
| `Team` | 프로젝트 팀, 스터디, 모임을 나타내는 aggregate root이다. 팀 정보와 팀 상태를 관리하며 멤버, 일정, 과제, 이벤트를 포함한다. |
| `TeamMember` | `Member`와 `Team`의 다대다 관계를 표현하는 association class이다. 팀 내 역할과 가입 일자를 저장한다. |
| `ScheduleBlock` | 멤버가 입력한 참석 불가능 시간 또는 일정 블록을 표현한다. 팀, 멤버, 요일, 시작 시간, 종료 시간을 가진다. |
...
| `Submission` | 특정 과제에 대해 멤버가 제출한 산출물을 표현한다. 제출자, 파일 URL, 제출 상태, 제출 시간을 가진다. |
| `FileAttachment` | 제출물에 첨부되는 파일 정보를 표현하는 value object이다. 파일명, 파일 URL, 업로드 시간을 관리한다. |
| `DataStore` | 도메인 객체의 저장, 조회, 수정 기능을 담당하는 repository 클래스이다. |
| `Event` | 참여 여부 투표가 필요한 모임 이벤트를 표현한다. 투표 마감 시간과 이벤트 상태를 관리한다. |
| `Vote` | 특정 이벤트에 대한 멤버의 참여, 불참, 미응답 상태를 표현한다. |
| `TeamWorkspace` | 이벤트 참여자로 확정된 멤버들이 사용하는 임시 워크스페이스를 표현한다. |
| `WorkspaceMessage` | 워크스페이스 안에서 작성되는 메시지를 표현한다. 작성자, 내용, 작성 시간을 가진다. |
